...
int swapColor(int c) {
    return (c==0)?1:0; //? is a ternary operator, if the condition is true, expression evaluates to 1, else 0
}
...
public:
    RBTNode(T data) //every node you insert starts as red
        : data(data),
          color(RED),
          parent(nullptr),
          left(nullptr),
...
public:
    ...
    void repair(RBTNode<T>* point);
    int height() const { return root->height(); }
};
 //all nodes are red or black, all new nodes are red (constructor with red) 
            //don't think I need to make code for this

        //root is black
        //red node can only have black children
        //all path from root to children must pass through same number of blapoint->parent->parentck nodes
template <class T>
void RBT<T>::repair(RBTNode<T>* point){
...
template <class T>
void RBT<T>::singleCR(RBTNode<T> *&point) {//clockwise
...
template <class T>
void RBT<T>::singleCCR(RBTNode<T> *&point) {//counter clockwise
    RBTNode<T> *grandparent = point;
...
template <class T>
void RBT<T>::insert(const T &toInsert, RBTNode<T> *&point, RBTNode<T> *parent) {//point points to root
    if (point == nullptr) {               // leaf location is found so insert node
        ...
        point->parent = parent;// sets parent pointer for the node

        RBTNode<T> *curr_node = point; // curr_node will be set appropriately when walking up the tree
        // TODO: ADD RBT RULES HERE
        repair(curr_node);//pass by reference bc i want to make changes to color
        //all nodes are red or black, all new nodes are red (constructor with red) 
            //don't think I need to make code for this

        //root is black
        //red node can only have black children
        //all path from root to children must pass through same number of black nodes
